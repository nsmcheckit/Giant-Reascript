...
--open GUI
retval, fileLoc= reaper.GetUserInputs("Read File", 1, "File Location ?extrawidth=150", "")
...
for i = 0, itemNum - 1, 1 do
  ...
  itemRegionName = "";
  itemNotes = "";
  itemNotesTableSplitArr = {}
  
  for j = 0, num_regions - 1, 1 do
    if ( reaper.GetMediaItemInfo_Value(itemIdArray[i],"D_POSITION") >= rgnStartArray[j] and reaper.GetMediaItemInfo_Value(itemIdArray[i],"D_POSITION")<= rgnEndArray[j])
      then itemRegionName = rgnNameArray[j];
    end
  item = reaper.GetMediaItem(0, i)
  itemTrackId = reaper.GetMediaItemInfo_Value(item,"P_TRACK");
  retval, trackName = reaper.GetTrackName(itemTrackId);
  itemTrackName = trackName;
  end
  
  if ( itemTrackName ~= "Dialogue Lines" )
    then 
    for k = 1,#noteJson,1 do
       --reaper.ShowConsoleMsg(noteJson[itemTrackName][k]["Audio File Name"]);
       if ( itemRegionName == noteJson[k]["Audio File Name"])
       then itemNotesTable = noteJson[k]
       end
     end
     
     itemNotesTableSplit = split(Utils.table2string(itemNotesTable),",")
     for l = 1, #itemNotesTableSplit, 1 do
       itemNotesTableSplitArr[l] = split(itemNotesTableSplit[l],"=")[2]
       itemNotes = itemNotes .. " " .. string.gsub(tostring(itemNotesTableSplitArr[l]),'"',"")
     end
     itemNotes = string.gsub(itemNotes,'}',"")
     reaper.ULT_SetMediaItemNote(item, itemNotes);
  end
end
end
...
reaper.ShowMessageBox("Set MetaData Successfully!", "Message", 0)
